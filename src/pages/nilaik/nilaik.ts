 ...
 */
 @Component({
 	selector: 'page-nilaik',
 	templateUrl: 'nilaik.html',
 })
 export class Nilaik {
 	...
 	slideNext(){
 		...
 		console.log(this.frmnilaik);
 		for(let i =0; i<this.kode.length;i++){
 			if(this.kode[i].id_matpel ==  this.frmnilaik.matpel){
 	...
 	lanjut(){
 		...
 		this.slides.slideNext();
 		for(let i =0; i<this.siswa.length;i++){
 			if(this.siswa[i].id_kelas ==  this.frmnilaik.kelas){
 				resultArray.push({
 					"id":this.siswa[i].id,
 	...
 	kirim(){
 		...
 		loader.present().then(()=>{
 			this.storage.get('token').then(token=>{
 				...
 				console.log(this.frmnilaik)
 				this.http.post(this.url2,JSON.stringify(this.frmnilaik),options).map(res => res.json())
 				.subscribe(data => {
