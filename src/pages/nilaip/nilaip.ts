 ...
 */
 @Component({
 	selector: 'page-nilaip',
 	templateUrl: 'nilaip.html',
 })
 export class Nilaip {
 	...
 	slideNext(){
 		...
 		console.log(this.frmnilaip);
 		for(let i =0; i<this.kode.length;i++){
 			if(this.kode[i].id_matpel ==  this.frmnilaip.matpel){
 	...
 	lanjut(){
 		...
 		this.slides.slideNext();
 		for(let i =0; i<this.siswa.length;i++){
 			if(this.siswa[i].id_kelas ==  this.frmnilaip.kelas){
 				resultArray.push({
 					"id":this.siswa[i].id,
 	...
 	kirim(){
 		...
 		loader.present().then(()=>{
 			this.storage.get('token').then(token=>{
 				...
 				console.log(this.frmnilaip)
 				this.http.post(this.url2,JSON.stringify(this.frmnilaip),options).map(res => res.json())
 				.subscribe(data => {
